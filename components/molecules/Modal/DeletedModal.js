...
const DeletedModal = ({isShown = false, setModalShown, cb}) => {
  return (
    <div 
      className={`z-[999] fixed ${isShown ? "flex ": "hidden "}items-center justify-center top-0 left-0 w-full h-full bg-black/50`}
      ...
      >
        <div className="max-w-md w-full bg-white px-5 py-3 rounded-lg shadow-lg">
            ...
            <div className="flex gap-4 mb-4">
                <Button 
                  className="text-black rounded-sm py-4 text-sm"
                  onClick={cb}
                  >yes</Button>
                <Button 
